...
import { doLogin, accountService } from '../../../../../services/api';
import moneyBank from './moneyBank';
import moneyCity from './moneyCity';
...
class BindCard extends React.Component {
  ...
  queryUserBaseInfo = async () => {
    let response = await doLogin.getUserBaseData();
    if (response.code === 0) {
  ...
  checkBank = async () => {
    ...
    const response = await accountService.verifyBankCard(bankCard.trim());
  ...
  bindBank = async () => {
    ...
    const response = await accountService.bindBankCard({
    ...
    return (
      <div className="pages" style={{ padding: '30px', width: '100%' }}>
        <div className="real_title_">
          <span className="safeCenter_" onClick={() => this.props.history.push('/index/uCenter/recharge')}>充值</span>
          <span style={{ fontSize: 16 }}> &gt; 绑定新银行卡</span>
        </div>
        <div className="forms">
